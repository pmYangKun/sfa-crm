...
## 2026-08-20 · memory 目录按规则体系重构（`9313f51`）

对齐南方电网项目的 4 段架构，把扁平的 `memory/` 拆成"状态 / 时间线 / 沉淀 / 规矩"四层：新增 `README.md`（入口 + 更新规则路由表）、`current.md`、`log.md`、`context/`（项目知识 + 内容策略）、`feedback/`。

- `project_main.md`（326 行）和 `MEMORY.md` **保留为重定向存根**——`specs/001~003/inputs/` 等历史档案仍指向它们，不改历史
- 顺带发现根 `README.md` 的"进度"表**已经过时**（写着 spec 005 待 brainstorm，实际 8-16 就上线了）→ 换成稳定的阶段概览。**高频变动的东西放在低频文件里必然烂掉**，这正是要拆的理由
- **本仓库是公开的**：从 memory 中移除服务器 IP、SSH 密钥路径、改密脚本路径，改为指向用户级 runbook
- 用户级 `project_sfacrm_content.md` 同步瘦成纯指针，执行细节全部下沉到本目录
